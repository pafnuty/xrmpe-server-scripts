    ...
    if ($null -ne $server.Process -and -not $server.Process.HasExited) {
        ...
        if ($isResponding) {
            ...
            if ($timeSinceLastResponse -gt $HangTimeout) {
                ...
                Write-DebugLog "Сервер $($server.Name) не отвечает в течение $([Math]::Round($timeSinceLastResponse)) секунд (порог: $HangTimeout)." -Color "Yellow" -Level "WARNING"
            }
        }
    }
}
...
# Функция для подготовки файлов данных сервера
...
Write-DebugLog "Подготовка файлов для сервера $($Server.Name) (ConfigPrefix: $($Server.ConfigPrefix))" -Color "Magenta" -Level "INFO"
...
if (-not (Test-Path -Path $serverDataPath)) {
    ...
    Write-DebugLog "Создана папка для данных сервера: $serverDataPath" -Color "Green" -Level "INFO"
}
...
if (-not (Test-Path -Path $userLtxPath)) {
    ...
    Write-DebugLog "Создание файла user.ltx: $userLtxPath" -Color "Magenta" -Level "INFO"

    # Проверяем, существует ли шаблон и путь к нему
    if (-not [string]::IsNullOrEmpty($TemplatesPath) -and (Test-Path -Path (Join-Path -Path $TemplatesPath -ChildPath "user.ltx"))) {
        ...
        Write-DebugLog "Файл user.ltx создан из шаблона: $userLtxTemplatePath" -Color "Green" -Level "INFO"
    } else {
        Write-Host "    ВНИМАНИЕ: Шаблон user.ltx не найден или путь к шаблонам не указан. Создание пустого файла." -ForegroundColor Yellow
        Write-DebugLog "Шаблон user.ltx не найден или путь к шаблонам не указан. Создание пустого файла." -Color "Yellow" -Level "WARNING"
...
if (-not (Test-Path -Path $bannedListPath)) {
    ...
    Write-DebugLog "Создание файла banned_list.ltx: $bannedListPath" -Color "Magenta" -Level "INFO"
...
if (-not (Test-Path -Path $bannedListIpPath)) {
    ...
    Write-DebugLog "Создание файла banned_list_ip.ltx: $bannedListIpPath" -Color "Magenta" -Level "INFO"
...
if ($Server.Radmins.Count -gt 0) {
    ...
    Write-DebugLog "Создание файла radmins.ltx: $radminsPath" -Color "Magenta" -Level "INFO"
...
function Find-ServerWindowsByPID {
  ...
  $callbackScript = {
      ...
      if ($windowClass -eq $global:WINDOW_CLASS_NAME -and $windowTitle -like $global:WINDOW_TITLE_PATTERN) {
          ...
          Write-DebugLog "Найдено окно STALKER: PID=$processId, Handle=$hwnd, Title='$windowTitle'" -Color "Green" -Level "INFO"
      } else {
          # Логируем информацию о других окнах только в режиме VERBOSE
          Write-DebugLog "Найдено окно: PID=$processId, Handle=$hwnd, Class=$windowClass, Title='$windowTitle'" -Color "Gray" -Level "VERBOSE"
      }
  ...
  Write-DebugLog "Начало поиска окон процессов..." -Color "Magenta" -Level "INFO"

  # Перечисляем все окна
  [void][Win32]::EnumWindows($enumWindowsCallback, [IntPtr]::Zero)

  # Выводим информацию о найденных окнах STALKER
  Write-DebugLog "Найдено окон STALKER: $($stalkerWindows.Count)" -Color "Magenta" -Level "INFO"
  ...
  if ($stalkerWindows.Count -gt 0) {
      ...
      for ($i = 0; $i -lt $global:ServersList.Count; $i++) {
          ...
          if ($null -ne $server.Process) {
              ...
              Write-DebugLog "Сервер $($server.Name): ProcessId=${procId}, HasExited=$($server.Process.HasExited)" -Color "Magenta" -Level "DEBUG"

              # Получаем командную строку процесса - логируем только в режиме VERBOSE
              $cmdLine = Get-ProcessCommandLine -processId $procId
              Write-DebugLog "Командная строка: $cmdLine" -Color "Magenta" -Level "VERBOSE"
              ...
              if ($null -ne $serverWindows -and $serverWindows.Count -gt 0) {
                  Write-DebugLog "Найдено окон STALKER для процесса ${procId}: $($serverWindows.Count)" -Color "Green" -Level "INFO"
                  ...
                  Write-DebugLog "Основное окно: Handle=$($mainWindow.Handle)" -Color "Green" -Level "INFO"

                  # Сохраняем дескриптор окна в объекте сервера
                  $global:ServersList[$i].WindowHandle = $mainWindow.Handle
              } else {
                  Write-DebugLog "Не найдено окон STALKER для процесса ${procId}" -Color "Yellow" -Level "WARNING"
              }
          } else {
              Write-DebugLog "Сервер $($server.Name): Process=null" -Color "Magenta" -Level "DEBUG"
          }
      }
      ...
      Write-DebugLog "Серверов без окон: $($serversWithoutWindows.Count), Оставшихся окон STALKER: $($remainingStalkerWindows.Count)" -Color "Magenta" -Level "INFO"
      ...
      for ($i = 0; $i -lt [Math]::Min($serversWithoutWindows.Count, $remainingStalkerWindows.Count); $i++) {
          ...
          Write-DebugLog "Назначение окна Handle=$($window.Handle) серверу $($server.Name)" -Color "Yellow" -Level "INFO"
          $global:ServersList[$serverIndex].WindowHandle = $window.Handle
      }
  } else {
      Write-DebugLog "Не найдено окон STALKER!" -Color "Red" -Level "ERROR"
  }
}
...
try {
  ...
  Write-DebugLog "Полный путь к игре: $fullGamePath" -Color "Magenta" -Level "INFO"
  Write-DebugLog "Полный путь к данным сервера: $fullServerDataPath" -Color "Magenta" -Level "INFO"
  ...
  Write-DebugLog "Вычисленный относительный путь: $relativePath" -Color "Green" -Level "INFO"
...
try {
    ...
    Write-DebugLog "Запуск сервера $($Server.Name) с аргументами: $processArgs" -Color "Magenta" -Level "INFO"
    $process = Start-Process -FilePath "dedicated\xrEngine.exe" -ArgumentList $processArgs -WorkingDirectory $BinPath -PassThru
    Write-DebugLog "Сервер $($Server.Name) запущен, ProcessId=$($process.Id)" -Color "Magenta" -Level "INFO"
    return $process
}
catch {
    $errorMsg = $_.Exception.Message
...
while (-not $moveSuccessful -and $attemptCount -lt $maxAttempts) {
    ...
    if ([Math]::Abs($rectAfter.Left - $X) -le 10 -and [Math]::Abs($rectAfter.Top - $Y) -le 10) {
        $moveSuccessful = $true
        Write-DebugLog "Окно успешно перемещено на указанную позицию (попытка $attemptCount)" -Color "Green" -Level "INFO"
    } else {
        Write-DebugLog "Попытка $attemptCount`: Окно не переместилось на указанную позицию" -Color "Yellow" -Level "WARNING"
        Write-DebugLog "Ожидаемая позиция: X=$X, Y=$Y; Фактическая позиция: X=$($rectAfter.Left), Y=$($rectAfter.Top)" -Color "Yellow" -Level "DEBUG"
...
if (-not $moveSuccessful) {
    Write-DebugLog "ВНИМАНИЕ: Не удалось переместить окно на указанную позицию после $maxAttempts попыток!" -Color "Red" -Level "ERROR"
    $rectAfter = Get-WindowRect -hwnd $WindowHandle
    Write-DebugLog "Ожидаемая позиция: X=$X, Y=$Y; Фактическая позиция: X=$($rectAfter.Left), Y=$($rectAfter.Top)" -Color "Red" -Level "ERROR"
}
...
if ($windowCount -gt 0) {
    ...
    for ($i = 0; $i -lt $global:ServersList.Count; $i++) {
        ...
        if ($null -ne $server.WindowHandle) {
            ...
            Write-DebugLog "Перемещение окна сервера $($server.Name): Handle=$hwnd" -Color "Magenta" -Level "INFO"
            Write-DebugLog "Новая позиция: X=$($position.X), Y=$($position.Y)" -Color "Magenta" -Level "DEBUG"
