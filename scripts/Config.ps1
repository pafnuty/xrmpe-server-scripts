...
try {
    ...
    # Выводим информацию о состоянии отладки только если она включена
    if ($global:DebugEnabled) {
        Write-Host "  Отладка включена" -ForegroundColor Magenta
    }
    
    if ($config.debugLogFile) {
        $global:DebugLogFile = Join-Path -Path $ScriptPath -ChildPath $config.debugLogFile
        if ($global:DebugEnabled) {
            Write-Host "  Файл журнала отладки: $global:DebugLogFile" -ForegroundColor Magenta
        }
    } else {
        $global:DebugLogFile = Join-Path -Path $ScriptPath -ChildPath "server_debug.log"
        if ($global:DebugEnabled) {
            Write-Host "  Используется файл журнала отладки по умолчанию: $global:DebugLogFile" -ForegroundColor Magenta
        }
    }
    ...
    if ($global:DebugEnabled) {
        ...
        # Создаем первую запись в журнале отладки
        Write-DebugLog "Запуск скрипта. Отладка включена." -Color "Magenta" -Level "INFO"
    }
